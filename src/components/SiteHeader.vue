<script>
    import { languageState, reopenCookieBanner } from "../assets/js/language.js";
    ...
    export default {
        ...
        methods: {
            ...
            // Il footer (che ha "Gestisci cookie") c'è solo in home e nelle pagine progetto:
            // da qui la scelta sui cookie si può cambiare da qualsiasi pagina.
            closeMenuAndFocus() {
                this.closeMenu();
        ...
        watch: {
            ...
            mobileMenuOpen(val) {
                ...
                // da tastiera o lettore di schermo il focus entra nel menu appena si apre
                if (val) this.$nextTick(() => this.$refs.mobMenu?.querySelector('a, button')?.focus());
            }
        },
        mounted() {
    ...
    <header>
        <nav class="navbar navbar-expand-md shadow-sm position-fixed w-100 top-0 z_index header_shadow bg_color"
            ...
            <div class="container d-flex justify-content-between align-items-center">
                ...
                <!-- Links sinistra -->
                <nav class="d-flex align-items-center" :aria-label="languageState.eng_lan ? 'Main navigation' : 'Navigazione principale'">
                ...
                <div class="logo_container d-none d-md-flex align-items-center">
                    <router-link :to="{ name: 'contacts' }" class="navbar-brand d-flex align-items-center justify-content-start mx-1 logo_filter scale_hover nav-icon-link" :class="{ 'nav-icon-link--active': $route.name === 'contacts' }" :title="languageState.eng_lan ? 'Contact' : 'Contatti'">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="22" height="40" fill="none" stroke="#212529" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                    </router-link>
                    <a class="navbar-brand align-items-center justify-content-start mx-0 logo_filter lighter scale_hover" rel="noopener noreferrer" :aria-label="languageState.eng_lan ? 'WhatsApp (opens in a new tab)' : 'WhatsApp (si apre in una nuova scheda)'"
                        ...
                        target="_blank">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="28" height="40" fill="#212529" viewBox="0 0 640 640">
                    ...
                    <a class="navbar-brand d-flex align-items-center justify-content-start mx-2 logo_filter lighter scale_hover" rel="noopener noreferrer" :aria-label="languageState.eng_lan ? 'LinkedIn (opens in a new tab)' : 'LinkedIn (si apre in una nuova scheda)'"
                        href="https://www.linkedin.com/in/damiano-casolari/" target="_blank">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="18" height="40" fill="#212529" class="bi bi-linkedin lighter" viewBox="0 0 16 16">
                    ...
                    <button type="button" @click="toggleLang" title="Language" :aria-label="languageState.eng_lan ? 'Passa all’italiano' : 'Switch to English'"
                        ...
                        <svg v-else aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="28" height="40" fill="#212529" viewBox="0 -960 960 960">
                    ...
                    <button type="button" @click="openCookieSettings"
                        ...
                        class="navbar-brand border-0 bg-transparent d-flex align-items-center justify-content-start ms-2 me-0 p-0 logo_filter lighter scale_hover">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="40" fill="none" stroke="#212529" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M12 2a10 10 0 1 0 10 10 4 4 0 0 1-5-5 4 4 0 0 1-5-5"/><path d="M8.5 8.5v.01"/><path d="M16 15.5v.01"/><path d="M12 12v.01"/><path d="M11 17v.01"/><path d="M7 14v.01"/></svg>
                    </button>
                </div>

                <!-- Hamburger — solo mobile -->
                <button ref="hamburger" class="hamburger_btn d-flex d-md-none" @click="toggleMenu" :aria-expanded="mobileMenuOpen" aria-controls="mobile-menu" aria-label="Menu">
                    <svg aria-hidden="true" class="ham-svg" :class="{ open: mobileMenuOpen }" width="20" height="20" viewBox="0 0 22 22" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" style="overflow:visible">
        ...
        <Transition name="mob-menu">
            <nav v-if="mobileMenuOpen" id="mobile-menu" ref="mobMenu" class="mob_menu" aria-label="Menu" @keydown.esc="closeMenuAndFocus">
                ...
                <div class="mob_menu__section">
                    <router-link :to="{ name: 'home' }" class="mob_menu__item" @click="closeMenu"
                        ...
                        <span aria-hidden="true" class="mob_menu__icon" style="background: rgba(0,0,0,0.06); color: #333;">
                    ...
                    <router-link :to="{ name: 'services' }" class="mob_menu__item" @click="closeMenu"
                        ...
                        <span aria-hidden="true" class="mob_menu__icon" style="background: rgba(0,0,0,0.09); color: #444;">
                    ...
                    <router-link :to="{ name: 'about' }" class="mob_menu__item" @click="closeMenu"
                        ...
                        <span aria-hidden="true" class="mob_menu__icon" style="background: rgba(0,0,0,0.07); color: #444;">
                ...
                <div class="mob_menu__section mob_menu__section--divided">
                     <router-link :to="{ name: 'contacts' }" class="mob_menu__item" @click="closeMenu"
                        ...
                        <span aria-hidden="true" class="mob_menu__icon" style="background: rgba(0,0,0,0.07); color: #555;">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                        </span>
                        <span>{{ languageState.eng_lan ? 'Contact' : 'Contatti' }}</span>
                    </router-link>
                    <a href="https://wa.me/393477952189?text=Buongiorno%2C%0Asono%20%5BNome%5D%20e%20vi%20contatto%20per%20avere%20informazioni%20sulla%20realizzazione%20di%20un%20sito%20web%20per%20la%20mia%20attivit%C3%A0.%0AResto%20in%20attesa%20di%20un%20vostro%20riscontro%2C%20grazie."
                    ...
                    <a href="https://www.linkedin.com/in/damiano-casolari/" target="_blank" rel="noopener noreferrer"
                        ...
                        <span aria-hidden="true" class="mob_menu__icon" style="background: rgba(0,0,0,0.08); color: #444;">
                            <svg width="15" height="15" viewBox="0 0 16 16" fill="currentColor"><path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/></svg>
                        </span>
                        <span>LinkedIn</span>
                    </a>
                    <button class="mob_menu__item" @click="toggleLang(); closeMenu()">
                        <span aria-hidden="true" class="mob_menu__icon" style="background: rgba(0,0,0,0.06); color: #555;">
                    ...
                    <button class="mob_menu__item" @click="openCookieSettings">
                        <span aria-hidden="true" class="mob_menu__icon" style="background: rgba(0,0,0,0.06); color: #555;">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a10 10 0 1 0 10 10 4 4 0 0 1-5-5 4 4 0 0 1-5-5"/><path d="M8.5 8.5v.01"/><path d="M16 15.5v.01"/><path d="M12 12v.01"/><path d="M11 17v.01"/><path d="M7 14v.01"/></svg>
                        </span>
                        <span>{{ languageState.eng_lan ? 'Manage cookies' : 'Gestisci cookie' }}</span>
                    </button>
                </div>
