...
export default {
    ...
    methods: {
        ...
        onBodyKey(event) {
            // in modalità offerta il corpo della card fa da pulsante: Invio e Spazio la aprono
    ...
    <Transition name="toast_slide">
        ...
        >
            <component
                ...
                v-bind="mode === 'wa' ? { href: waLink, target: '_blank', rel: 'noopener noreferrer' } : { role: 'button', tabindex: 0 }"
                @click="mode === 'offer' && openModal()"
                @keydown.enter="onBodyKey"
                @keydown.space="onBodyKey"
            >
                <div class="offer_toast_badge" aria-hidden="true">
            ...
            <button v-if="mode === 'offer' || mode === 'wa'" class="offer_toast_close" @click.stop="dismiss" :aria-label="languageState.eng_lan ? 'Close' : 'Chiudi'">×</button>
        </div>
    </Transition>
</template>
